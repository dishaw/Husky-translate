...
class LLMThread(models.Model):
    ...
    def generate_messages(self, last_message):
        ...
        # Continue generation loop (with max iterations to prevent infinite loops)
        max_iterations = 20  # Safety limit: max 20 tool call round-trips
        iteration = 0
        while self._should_continue(last_message):
            iteration += 1
            if iteration > max_iterations:
                _logger.warning(
                    f"Thread {self.id}: Max iterations ({max_iterations}) reached, "
                    f"stopping tool call loop."
                )
                # Post a warning message to the thread
                self.message_post(
                    body=_("⚠️ 已达到最大工具调用次数 (%s)，自动停止。") % max_iterations,
                    llm_role="assistant",
                    author_id=False,
                )
                break

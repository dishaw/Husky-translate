...
class LLMProvider(models.Model):
    ...
    proxy_url = fields.Char(
        string="Proxy URL",
        help="HTTP/SOCKS5 proxy URL for API requests, e.g., socks5://vpn:1080 or http://proxy:8080",
    )
    ...
    @property
    def client(self):
        """Get client instance - each service provider must implement {service}_get_client() method"""
        if not self.service:
            raise UserError(_("Provider service not configured"))
        
        service_method = f"{self.service}_get_client"
        if not hasattr(self, service_method):
            raise NotImplementedError(
                _("Method '%s' not implemented for service '%s'")
                % (service_method, self.service),
            )
        
        return getattr(self, service_method)()
    ...
    def action_fetch_models(self):
        ...
        for model_data in models_data:
            ...
            if not name:
                _logger.warning("Skipping model with no name: %s", model_data)
                continue

...
class LLMThread(models.Model):
    ...
    def _process_llm_body(self, body):
        ...
        Also converts thinking content to styled div for real-time display.
        Supports:
        - <think>...</think> tags (Claude, etc.)
        - Thinking...\n...\ndone thinking (DeepSeek R1)
        - LaTeX math expressions (block and inline)
        
        LaTeX expressions are protected from markdown processing to prevent
        ...
        def save_latex_block(content):
            """Save LaTeX content and return placeholder"""
            latex_blocks.append(content)
            return f'LATEXPLACEHOLDER{len(latex_blocks) - 1}END'
        
        # Extract LaTeX expressions in order of specificity (longest first)
        # Pattern 1: Display mode with \[ ... \]
        body = re.sub(r'\\\[([^\]]*)\\\]', lambda m: save_latex_block(m.group(0)), body, flags=re.DOTALL)
        
        # Pattern 2: Display mode with $$ ... $$
        body = re.sub(r'\$\$([^\$]*?)\$\$', lambda m: save_latex_block(m.group(0)), body, flags=re.DOTALL)
        
        # Pattern 3: \begin{...} ... \end{...} (for \begin{array}, \begin{equation}, etc.)
        body = re.sub(
            r'\\begin\{([^}]+)\}(.*?)\\end\{\1\}',
            lambda m: save_latex_block(m.group(0)),
            body,
            flags=re.DOTALL
        )
        
        # Pattern 4: Inline mode with \( ... \)
        body = re.sub(r'\\\(([^\)]*)\\\)', lambda m: save_latex_block(m.group(0)), body, flags=re.DOTALL)
        
        # Pattern 5: Inline mode with $ ... $
        body = re.sub(r'(?<!\$)\$([^\$]+?)\$(?!\$)', lambda m: save_latex_block(m.group(0)), body, flags=re.DOTALL)
        
        # ============================================================================
        # STEP 2: Extract and protect think blocks
        # ============================================================================
        
        def save_think_block(content):
            """Save think content and return placeholder"""
            # Convert newlines to <br> for think blocks (no markdown processing)
            content_formatted = content.replace('\n', '<br/>')
            think_blocks.append(content_formatted)
            return f'THINKPLACEHOLDER{len(think_blocks) - 1}END'
        
        # Pattern 1: <think>...</think> tags (Claude, etc.)
        body = re.sub(r'<think>(.*?)</think>', lambda m: save_think_block(m.group(1)), body, flags=re.DOTALL)

        
        # Pattern 2: Thinking...\n...\ndone thinking (DeepSeek R1)
        body = re.sub(
            r'Thinking\.\.\.\n(.*?)\ndone thinking',
            lambda m: save_think_block(m.group(1)),
            body,
            flags=re.DOTALL | re.IGNORECASE
        )
        
        # Handle incomplete think blocks for streaming
        # Pattern 1: <think> without closing tag
        incomplete_think = re.search(r'<think>(.*?)$', body, flags=re.DOTALL)
        if incomplete_think:
            content = incomplete_think.group(1).replace('\n', '<br/>')
            body = body[:incomplete_think.start()] + f'THINKPLACEHOLDER{len(think_blocks)}END'
            think_blocks.append(content)
        
        # Pattern 2: Thinking... without "done thinking"
        incomplete_ds = re.search(r'Thinking\.\.\.\n(.*?)$', body, flags=re.DOTALL | re.IGNORECASE)
        if incomplete_ds:
            content = incomplete_ds.group(1).replace('\n', '<br/>')
            body = body[:incomplete_ds.start()] + f'THINKPLACEHOLDER{len(think_blocks)}END'
            think_blocks.append(content)
        
        # ============================================================================
        # STEP 3: Process remaining content with markdown and emoji
        # ============================================================================
        
        # First, normalize emoji codes (handle both : format and / separators)
        # Convert :emoji_name: or :emoji/name: to actual emoji
        import re as re_module
        
        # Normalize emoji codes: replace / with _ for consistency
        body = re_module.sub(r':([a-zA-Z_/]+):', lambda m: ':' + m.group(1).replace('/', '_') + ':', body)
        
        # Process markdown (enable tables extra for proper table rendering)
        body = markdown2.markdown(body, extras=['tables', 'fenced-code-blocks'])
        
        # Convert emoji codes to actual emoji (emoji.emojize handles :emoji_name: format)
        body = emoji.emojize(body, language='alias')
        
        # ============================================================================
        # STEP 3.5: Post-process HTML tables - add wrapper and copy button
        # ============================================================================
        import re as re_mod
        
        def _add_table_wrapper(match):
            """Wrap HTML table with a container div and copy-to-Excel button"""
            table_html = match.group(0)
            return (
                '<div class="llm-table-wrapper">'
                '<div class="llm-table-toolbar">'
                '<button class="llm-table-copy-btn" title="复制表格到剪贴板 (可粘贴到 Excel)">'
                '<i class="fa fa-clipboard"></i> 复制到 Excel'
                '</button>'
                '</div>'
                f'{table_html}'
                '</div>'
        ...
        body = re_mod.sub(r'<table>.*?</table>', _add_table_wrapper, body, flags=re_mod.DOTALL)
        
        # ============================================================================
        # STEP 4: Restore protected content (LaTeX and think blocks)
        # ============================================================================
        
        # Restore LaTeX blocks
        for i, latex_content in enumerate(latex_blocks):
            placeholder = f'LATEXPLACEHOLDER{i}END'
            # Wrap LaTeX for KaTeX client-side rendering
            # Use data attribute to store the LaTeX for JS to process
            latex_html = f'<div class="llm-math" data-latex="true">{latex_content}</div>'
            # Handle both <p> wrapped and unwrapped cases
            if f'<p>{placeholder}</p>' in body:
                body = body.replace(f'<p>{placeholder}</p>', latex_html)
            else:
                body = body.replace(placeholder, latex_html)
        
        # Restore think blocks
        for i, content in enumerate(think_blocks):
            placeholder = f'THINKPLACEHOLDER{i}END'
            think_div = f'<div class="llm-think">{content}</div>'
            # Handle both <p> wrapped and unwrapped cases
            if f'<p>{placeholder}</p>' in body:
                body = body.replace(f'<p>{placeholder}</p>', think_div)
            else:
                body = body.replace(placeholder, think_div)

    ...
    start(env, { orm, "mail.store": mailStore, notification }) {
        const llmStore = reactive({
            ...
            async startLLMStreaming(threadId, message, attachmentIds = []) {
                ...
                try {
                    // POST the message first to avoid URL length limits,
                    // then open SSE stream with a short token
                    ...
                    if (message || attachmentIds.length > 0) {
                        try {
                            ...
                            if (prepareResult.token) {
                                url = `/llm/thread/generate?thread_id=${threadId}&token=${prepareResult.token}`;
                            }
                        } catch (e) {
                            // Fallback: pass short messages via URL (backward compat)
                            console.warn("prepare_message failed, falling back to URL params:", e);
                            if (message) {
            ...
            async createNewThread({ recordModel, recordId } = {}) {
                ...
                const firstModel = this.getFirstAvailableModelForProvider(firstProvider.id);

                if (!firstModel) {
                    notification.add(
                        _t(
                            "No AI models are available for the selected provider. Please contact your administrator to configure AI models.",
                        ),
                        { type: "danger" },
                    );
                    return;
            ...
            // Get first available model for a specific provider
            // Prioritizes default models, then falls back to first available
            getFirstAvailableModelForProvider(providerId) {
                const models = Array.from(this.llmModels.values()).filter(
                    (model) => model.provider_id && model.provider_id[0] === providerId
                );
                ...
                // Try to find a default model for chat/multimodal first
                const defaultModel = models.find(
                    (m) => m.default && (m.model_use === 'chat' || m.model_use === 'multimodal')
                );
                if (defaultModel) {
                    return defaultModel;
                }
                
                // Fall back to first chat or multimodal model
                const chatModel = models.find(
                    (m) => m.model_use === 'chat' || m.model_use === 'multimodal'
                );
                if (chatModel) {
                    return chatModel;
                }
            ...
            // Get first available model (deprecated - use getFirstAvailableModelForProvider)

...
export class LLMChatContainer extends Component {
  ...
  async deleteThread(threadId) {
    const thread = this.llmStore.llmThreadList.find(t => t.id === threadId);
    if (!thread) {
      console.warn("[LLMChatContainer] Thread not found:", threadId);
      return;
    }

    // Show confirmation dialog
    if (!window.confirm(`Are you sure you want to delete the conversation "${thread.name}"?`)) {
      return;
    }

    try {
      // Check if we need to select another thread after deletion
      const wasActive = this.activeThread?.id === threadId;
      const remainingThreads = this.filteredThreadList.filter(t => t.id !== threadId);
      
      // Delete the thread via ORM service
      await this.orm.unlink("llm.thread", [threadId]);

      // Remove from mailStore Thread records to update UI immediately
      // Find the exact key in records (could be "llm.thread_<id>" format)
      if (this.mailStore.Thread?.records) {
        Object.keys(this.mailStore.Thread.records).forEach((key) => {
          const record = this.mailStore.Thread.records[key];
          if (record?.model === "llm.thread" && record?.id === threadId) {
            delete this.mailStore.Thread.records[key];
          }
        });
      }

      // If deleted thread was active, select another thread or clear
      if (wasActive) {
        if (remainingThreads.length > 0) {
          await this.llmStore.selectThread(remainingThreads[0].id);
        } else {
          // No more threads, clear the active thread
      ...
      console.error("[LLMChatContainer] Error deleting thread:", error);

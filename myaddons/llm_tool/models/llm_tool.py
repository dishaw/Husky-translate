        ...
        try:
            with self.env.cr.savepoint(flush=False):
                ...
                if existing:
                    ...
                    # Check for name collision with a tool from a different model
                    name_collision = self.with_context(active_test=False).search(
                        [("name", "=", tool_name)],
                        limit=1,
                    )
                    if name_collision:
                        _logger.warning(
                            "Tool name '%s' from %s.%s conflicts with existing tool "
                            "'%s' (id=%s, model=%s.%s). Skipping registration. "
                            "Rename one of the tools to avoid the conflict.",
                            tool_name,
                            model_name,
                            method_name,
                            name_collision.name,
                            name_collision.id,
                            name_collision.decorator_model,
                            name_collision.decorator_method,
                        )
                    else:
            ...
            # Check if this is a unique constraint violation for tool name
            error_msg = str(e)
            if "unique_tool_name" in error_msg or "unique_function_tool" in error_msg:
                _logger.warning(
                    "Duplicate tool registration for %s.%s (name='%s'): %s",
                    model_name,
                    method_name,
                    tool_name,
                    e,
